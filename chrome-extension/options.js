...
function displayBlockedSites() {
  chrome.storage.local.get([BLOCKED_SITES_KEY], (result) => {
    ...
    sites.forEach(site => {
      ...
      li.innerHTML = `
        <span>${site}</span>
        <a href="#" class="secondary-content" onclick="removeBlockedSite('${site}')">
...
function displayCustomQuotes() {
  chrome.storage.local.get([CUSTOM_QUOTES_KEY], (result) => {
    ...
    quotes.forEach((quote, index) => {
      ...
      li.innerHTML = `
        <span>${quote}</span>
        <a href="#" class="secondary-content" onclick="removeCustomQuote(${index})">
          <i class="fas fa-trash red-text"></i>
        </a>
      `;
  ...
  chrome.storage.local.get([AI_TOOLS_KEY], (result) => {
    ...
    tools.forEach((tool, index) => {
      ...
      li.innerHTML = `
        ...
        <a href="#" class="secondary-content" onclick="removeAITool(${index})">
          <i class="fas fa-trash red-text"></i>
        </a>
      `;
