...
function setupEventListeners() {
  ...
  if (importBookmarksBtn) importBookmarksBtn.addEventListener('click', importBookmarks);
    ...
    container.innerHTML = notes.map(note => `
      <div class="note-item">
        <div class="note-header">
          ...
          <button class="remove-btn" onclick="deleteNote(${note.id})">
            <i class="fas fa-trash"></i>
          </button>
        </div>
      ...
      return `
        <div class="goal-item ${isCompleted ? 'completed' : ''}">
          ...
          <div class="goal-actions">
            ${!isCompleted ? `
              <button class="goal-btn" onclick="updateGoalProgress(${goal.id}, 1)">
...
function loadDashboardGoals() {
  chrome.storage.local.get(['goals'], (result) => {
    ...
    if (activeGoals.length === 0) {
      container.innerHTML = '<div class="no-data">No active goals. <a href="#" onclick="switchToGoals()" style="color: #4ade80;">Add your first goal!</a></div>';
      return;
    }
